...
const unsigned long CameraPeriod = 30000;
#define joyX A0
#define joyY A1
...
void printMenu(void) {
  ...
  Serial.println(F("[K] Start the Video Recorder"));
...
void loop() {
  ...
  yMap = map(xValue, 0,1023, 0, 90);
  Serial.print(xMap);

  if (currentMillis - CameraMillis >= CameraPeriod)
  {
    // read the time
    char buffer[23];
    ...
    //checkCamera(String(buffer), String(duration));
    CameraMillis = currentMillis; 
  }
  ...
  switch (command) {
    case 'K':{
      // send an SMS!
        char duration[21], message[141];
        flushSerial();
        Serial.print(F("Input Duration in Seconds:"));
        readline(duration, 20);
        ...
        Serial.println("Start_" + String(duration) + "_" + String(buffer));  //Record Video for 3600s = 1hr.
...
void checkCamera(String(time), String(duration)){
  Serial.println("Start_" + duration + "_" + time);  //Record Video for 3600s = 1hr.
}
